...
public struct FirewallDataSource: DataSource {
    ...
    func parseALFPlist(_ plist: [String: Any]) -> FirewallStatus {
        ...
        let allowSigned = (plist["allowsignedenabled"] as? Int ?? 0) != 0

        // allowdownloadsignedenabled is the "automatically allow built-in" toggle
        let allowBuiltIn = (plist["allowdownloadsignedenabled"] as? Int ?? 0) != 0
