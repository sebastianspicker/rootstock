...
macOS-only inventories may omit. An AD group mapped to a macOS local group can
create transitive relationships to host permissions and application evidence.
Rootstock records those relationships so queries can include the imported
identity context.

Kerberos credential artifacts (ccache files, keytab files, krb5.conf) are
security-relevant. Unexpected read access to a ccache or keytab is a finding
that requires validation of file contents, ticket state, principal scope, and
KDC policy. Rootstock records metadata and does not read credential material.
...
Security control: The ZIP parser rejects a JSON member whose declared
decompressed size exceeds 100 MB. This is a per-member limit, not a complete
archive resource budget or general ZIP-bomb defense.

### OpenGraph Export: opengraph_export.py

Exports supported mapped Rootstock node and edge types in BloodHound CE v8+
OpenGraph JSON format. Each node type maps to a
...
usernames. The current rule favors identifying candidate links over omitting
