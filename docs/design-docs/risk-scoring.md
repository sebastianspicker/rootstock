...
The weights are model parameters used to separate score tiers:

- Injection methods (3.0): This is the highest configured factor because
  a modeled injection condition can connect an application to its existing
  privileges. The factor alone places an application in the model's medium
  tier; it does not establish that injection succeeds.

- Full Disk Access (2.0): The model assigns this TCC grant a high weight
  because it permits broad protected-file access within current macOS policy.
  Actual access still depends on the process identity, file permissions, and
  operating-system behavior. Combined with injection, the configured score is
  5.0 (high).
