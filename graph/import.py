...
_REL_TYPES = [
    # Import-created
    "HAS_TCC_GRANT", "HAS_ENTITLEMENT", "SIGNED_BY_SAME_TEAM",
    "COMMUNICATES_WITH", "PERSISTS_VIA", "RUNS_AS", "CAN_HIJACK",
    "CAN_READ_KEYCHAIN", "CONFIGURES",
    "MEMBER_OF", "ACCESSIBLE_BY", "HAS_FIREWALL_RULE",
    "HAS_SESSION", "SUDO_NOPASSWD",
    "INSTALLED_ON", "LOCAL_TO",
    "SIGNED_BY_CA", "ISSUED_BY",
    "PAIRED_WITH",
    # Inference-created
    "CAN_INJECT_INTO", "CHILD_INHERITS_TCC",
    "CAN_SEND_APPLE_EVENT", "HAS_TRANSITIVE_FDA",
    "CAN_WRITE", "PROTECTS", "CAN_MODIFY_TCC", "CAN_INJECT_SHELL",
    "CAN_CONTROL_VIA_A11Y", "CAN_BLIND_MONITORING", "CAN_DEBUG",
    "MDM_OVERGRANT", "SHARES_KEYCHAIN_GROUP", "CAN_CHANGE_PASSWORD",
    "MAPPED_TO", "FOUND_ON", "HAS_KERBEROS_CACHE", "HAS_KEYTAB",
    "CAN_READ_KERBEROS", "AD_USER_OF",
    "HAS_SANDBOX_PROFILE", "CAN_ESCAPE_SANDBOX", "CAN_ACCESS_MACH_SERVICE",
]
...
def main() -> int:
    ...
    print(f"  groups:   {len(scan.local_groups)}")
    print(f"  remote:   {len(scan.remote_access_services)}")
    print(f"  firewall: {len(scan.firewall_status)}")
    print(f"  sessions: {len(scan.login_sessions)}")
    print(f"  auth_rights: {len(scan.authorization_rights)}")
    print(f"  auth_plugins: {len(scan.authorization_plugins)}")
    print(f"  sysext:   {len(scan.system_extensions)}")
    print(f"  sudoers:  {len(scan.sudoers_rules)}")
    print(f"  procs:    {len(scan.running_processes)}")
    print(f"  fileacls: {len(scan.file_acls)}")
    print(f"  bt_devs:  {len(scan.bluetooth_devices)}")
    print(f"  ad_bind:  {'yes' if scan.ad_binding and scan.ad_binding.is_bound else 'no'}")
    print(f"  kerberos: {len(scan.kerberos_artifacts)}")
    print(f"  sandbox:  {len(scan.sandbox_profiles)}")
    ...
    with driver.session() as session:
        ...
        n_cas, n_signed_by, n_issued_by = import_certificate_authorities(session, scan.applications)
        print(f"  Cert authorities: {n_cas} nodes, {n_signed_by} SIGNED_BY_CA, {n_issued_by} ISSUED_BY edges")
        ...
        n_items, n_persists, n_runs, n_hijack = import_launch_items(session, scan.launch_items)
        print(f"  Launch items:  {n_items} nodes, {n_persists} PERSISTS_VIA, {n_runs} RUNS_AS, {n_hijack} CAN_HIJACK edges")
        ...
        n_sessions, n_has_session = import_login_sessions(session, scan.login_sessions)
        ...
        n_auth_plugins = import_authorization_plugins(session, scan.authorization_plugins)
        ...
        n_bt, n_paired = import_bluetooth_devices(session, scan.bluetooth_devices, scan.hostname)
        print(f"  BT devices:    {n_bt} nodes, {n_paired} PAIRED_WITH edges")

        n_adgroups, n_mapped = import_ad_binding(session, scan.ad_binding, scan.hostname)
        ...
        print(f"  Kerberos:      {n_ka} artifacts, {n_found} FOUND_ON, {n_cache} HAS_KERBEROS_CACHE, {n_kt} HAS_KEYTAB edges")

        n_sandbox, n_sandbox_edges = import_sandbox_profiles(session, scan.sandbox_profiles)
    ...
    print(
        f"  Nodes:  {node_counts['Application']} apps, {node_counts['Entitlement']} entitlements, "
        f"{node_counts['XPC_Service']} XPC, {node_counts['LaunchItem']} launch items, "
        f"{node_counts['Keychain_Item']} keychain, {node_counts['MDM_Profile']} MDM"
    )
    print(
