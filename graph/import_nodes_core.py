...
def import_computer(
    session: Session, computer: ComputerData,
...
def import_local_to(session: Session, hostname: str, scan_id: str) -> int:
    """Create LOCAL_TO edges from Users linked to this scan's data to the Computer node. Returns edge count."""
    result = session.run(
        """
        MATCH (li:LaunchItem)<-[:PERSISTS_VIA]-(a:Application {scan_id: $scan_id})
        MATCH (li)-[:RUNS_AS]->(u:User)
        WITH DISTINCT u
