...
def import_login_sessions(
    session: Session, sessions: list[LoginSessionData]
) -> tuple[int, int]:
    """
    ...
    # MERGE LoginSession nodes (terminal is unique per active session)
    session.run(
        """
        UNWIND $records AS r
        MERGE (s:LoginSession {terminal: r.terminal})
    ...
    result = session.run(
        """
        UNWIND $records AS r
        MATCH (s:LoginSession {terminal: r.terminal})
