...
def import_scan(session, scan: ScanResult) -> None:
    ...
    import_login_sessions(session, scan.login_sessions)
    ...
    print(f"  [{hostname}] {n_apps} apps, {grants_linked} grants, "
...
def main() -> int:
    ...
    if len(set(hostnames)) != len(hostnames):
        print(f"WARNING: Duplicate hostnames detected: {hostnames}", file=sys.stderr)
