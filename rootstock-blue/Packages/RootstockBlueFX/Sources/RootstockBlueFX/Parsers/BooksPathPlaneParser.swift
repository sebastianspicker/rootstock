...
/// Books / EPUB path residual plane markers (Wave-16 red↔blue pair).
/// Honesty: never extracts EPUB contents or Books annotations as bulk export.
public struct BooksPathPlaneParser: ArtifactParser {
    public let manifest = PluginManifest(id: "BKPATH", tier: .tier2, description: "Books path plane markers")
    public init() {}
    public func parse(source: ImageSource) throws -> [EventEnvelope] {
        let root = ArtifactRoot(source: source)
        var events: [EventEnvelope] = []
        var seen = PathDeduper()
        for rel in ["Library/Preferences/books_path_plane.json", "Library/Logs/books_path_plane.jsonl"] {
    ...
    private func makeEvent(from item: [String: Any], sourceURL: URL) -> EventEnvelope? {
        ...
        if !risk.contains("books_surface") { risk.append("books_surface") }
        let user = stringish(item["user"]) ?? inferUser(from: path) ?? inferUser(from: sourceURL.path) ?? ""
        var fields: [String: String] = [
            "bkpath.path": path, "bkpath.name": name,
            "bkpath.notes": stringish(item["notes"]) ?? "Books path plane markers - never extracts EPUB contents or Books annotations as bulk export",
            "bkpath.secrets_exported": "false",
            FieldTaxonomy.eventType: "books.path", FieldTaxonomy.userName: user,
        ]
