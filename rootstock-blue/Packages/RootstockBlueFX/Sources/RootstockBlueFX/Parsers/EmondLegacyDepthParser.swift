...
public struct EmondLegacyDepthParser: ArtifactParser {
    ...
    public func parse(source: ImageSource) throws -> [EventEnvelope] {
        ...
        for rel in ["Library/Preferences/emond_legacy_depth.json", "Library/Logs/emond_legacy_depth.jsonl"] {
        ...
        for url in root.enumerate(matching: { url in
            ...
            return n == "emond_legacy_depth.json" || n == "emond_legacy_depth.jsonl"
        }) {
    ...
    private func makeEvent(from item: [String: Any], sourceURL: URL) -> EventEnvelope? {
        ...
        if !risk.contains("emond_surface") { risk.append("emond_surface") }
        let user = stringish(item["user"]) ?? inferUser(from: path) ?? inferUser(from: sourceURL.path) ?? ""
        var fields: [String: String] = [
            "emondleg.path": path, "emondleg.name": name,
            "emondleg.notes": stringish(item["notes"]) ?? "Emond legacy depth markers - never installs emond rules or enables the legacy event monitor daemon",
            "emondleg.secrets_exported": "false",
            FieldTaxonomy.eventType: "emond.legacy", FieldTaxonomy.userName: user,
        ]
        if !risk.isEmpty { fields["emondleg.risk_tags"] = risk.joined(separator: ",") }
        return EventEnvelope(
            eventTime: parseDate(item["timestamp"] ?? item["seen_at"]) ?? Date(),
            collectedAt: Date(), source: .parser, sourcePlugin: "EMONDLEGACY",
            eventType: "emond.legacy",
            entityRefs: [EntityID(kind: .host, value: "emondleg|\(name.isEmpty ? path : name)")],
