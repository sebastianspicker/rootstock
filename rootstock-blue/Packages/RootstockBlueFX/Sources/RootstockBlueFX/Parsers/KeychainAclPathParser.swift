...
public struct KeychainAclPathParser: ArtifactParser {
    ...
    public func parse(source: ImageSource) throws -> [EventEnvelope] {
        ...
        for rel in ["Library/Preferences/keychain_acl_path.json", "Library/Logs/keychain_acl_path.jsonl"] {
    ...
    private func makeEvent(from item: [String: Any], sourceURL: URL) -> EventEnvelope? {
        ...
        if !risk.contains("keychain_acl_surface") { risk.append("keychain_acl_surface") }
        let user = stringish(item["user"]) ?? inferUser(from: path) ?? inferUser(from: sourceURL.path) ?? ""
        var fields: [String: String] = [
            "kcacl.path": path, "kcacl.name": name,
            "kcacl.notes": stringish(item["notes"]) ?? "Keychain ACL path plane markers - never dumps keychain items, passwords, or private keys",
            "kcacl.secrets_exported": "false",
            FieldTaxonomy.eventType: "keychain.acl_path", FieldTaxonomy.userName: user,
        ]
