...
public struct PhotosLibraryPathParser: ArtifactParser {
    ...
    public func parse(source: ImageSource) throws -> [EventEnvelope] {
        ...
        for rel in ["Library/Preferences/photos_library_path.json", "Library/Logs/photos_library_path.jsonl"] {
        ...
        for url in root.enumerate(matching: { url in
            ...
            return n == "photos_library_path.json" || n == "photos_library_path.jsonl"
        }) {
    ...
    private func makeEvent(from item: [String: Any], sourceURL: URL) -> EventEnvelope? {
        ...
        if !risk.contains("photos_surface") { risk.append("photos_surface") }
        let user = stringish(item["user"]) ?? inferUser(from: path) ?? inferUser(from: sourceURL.path) ?? ""
        var fields: [String: String] = [
            "photoslib.path": path, "photoslib.name": name,
            "photoslib.notes": stringish(item["notes"]) ?? "Photos library path plane markers - never reads photo contents or exports Photo Library media",
            "photoslib.secrets_exported": "false",
            FieldTaxonomy.eventType: "photos.library", FieldTaxonomy.userName: user,
        ]
        if !risk.isEmpty { fields["photoslib.risk_tags"] = risk.joined(separator: ",") }
        return EventEnvelope(
            eventTime: parseDate(item["timestamp"] ?? item["seen_at"]) ?? Date(),
            collectedAt: Date(), source: .parser, sourcePlugin: "PHOTOSLIBRARY",
            eventType: "photos.library",
            entityRefs: [EntityID(kind: .host, value: "photoslib|\(name.isEmpty ? path : name)")],
