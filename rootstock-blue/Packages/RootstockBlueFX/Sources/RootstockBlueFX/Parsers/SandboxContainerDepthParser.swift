...
public struct SandboxContainerDepthParser: ArtifactParser {
    ...
    public func parse(source: ImageSource) throws -> [EventEnvelope] {
        ...
        for rel in ["Library/Preferences/sandbox_container_depth.json", "Library/Logs/sandbox_container_depth.jsonl"] {
        ...
        for url in root.enumerate(matching: { url in
            ...
            return n == "sandbox_container_depth.json" || n == "sandbox_container_depth.jsonl"
        }) {
    ...
    private func makeEvent(from item: [String: Any], sourceURL: URL) -> EventEnvelope? {
        ...
        if !risk.contains("sandbox_surface") { risk.append("sandbox_surface") }
        let user = stringish(item["user"]) ?? inferUser(from: path) ?? inferUser(from: sourceURL.path) ?? ""
        var fields: [String: String] = [
            "sbxctr.path": path, "sbxctr.name": name,
            "sbxctr.notes": stringish(item["notes"]) ?? "Sandbox container depth markers - never breaks app sandbox or forges container entitlements",
            "sbxctr.secrets_exported": "false",
            FieldTaxonomy.eventType: "sandbox.container", FieldTaxonomy.userName: user,
        ]
        if !risk.isEmpty { fields["sbxctr.risk_tags"] = risk.joined(separator: ",") }
        return EventEnvelope(
            eventTime: parseDate(item["timestamp"] ?? item["seen_at"]) ?? Date(),
            collectedAt: Date(), source: .parser, sourcePlugin: "SANDBOXCONTAINER",
            eventType: "sandbox.container",
            entityRefs: [EntityID(kind: .host, value: "sbxctr|\(name.isEmpty ? path : name)")],
