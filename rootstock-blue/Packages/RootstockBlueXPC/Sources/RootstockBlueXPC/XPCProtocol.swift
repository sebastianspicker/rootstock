...
public struct LocalXPCStub: RootstockBlueXPCServicing {
    ...
    public func handle(_ request: XPCRequest) async throws -> XPCResponse {
        ...
        case .collectPack:
            return XPCResponse(id: request.id, ok: true, message: "stub: collect \(request.packName ?? "?")")
        case .exportCase:
            return XPCResponse(id: request.id, ok: true, message: "stub: export \(request.exportPath ?? "?")")
