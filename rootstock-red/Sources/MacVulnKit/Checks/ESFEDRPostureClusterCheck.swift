...
public struct ESFEDRPostureClusterCheck: Check {
    ...
    private static func thinSensorRemote(state: CollectedState) -> Finding? {
        ...
        return Finding(
            id: "\(id).thin_sensor_remote",
            title: "ESF/EDR cluster: no third-party sensor path hits with remote access enabled",
            severity: .medium,
            confidence: .low,
            ...
            falsePositiveNotes: "Agents may exist as system extensions without catalog hits",
            dryRunSafe: true,
            opsecScore: 14,
            esfExpected: ["OPEN"]
        )
    ...
    private static func sysextWithoutProductCatalog(state: CollectedState) -> Finding? {
        ...
        return Finding(
            id: "\(id).sysext_without_catalog",
            title: "ESF/EDR cluster: system extensions present but security product catalog empty",
            severity: .low,
            confidence: .low,
            category: .securityProduct,
            evidence: [
            ...
            attackTechniques: ["T1518.001", "T1547"],
            remediation: [
    ...
    private static func frameworkOnlyNoClients(state: CollectedState) -> Finding? {
        ...
        return Finding(
            id: "\(id).framework_only_high_value",
            title:
                "ESF/EDR cluster: EndpointSecurity framework present without third-party clients on high-value host",
            severity: .low,
            confidence: .low,
            category: .securityProduct,
            evidence: [
            ...
            falsePositiveNotes:
                "Framework + endpointsecurityd presence is expected on stock macOS even without third-party EDR",
            dryRunSafe: true,
            opsecScore: 10,
            esfExpected: ["OPEN"]
        )
