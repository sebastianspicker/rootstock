...
public struct AuthRightsPrivilegeSurfaceVector: Check {
    ...
    public func evaluate(state: CollectedState, context: EvaluationContext) async throws -> [Finding] {
        guard let auth = state.authRights else {
            // Fall back to collector notes if typed state absent
            guard state.collectorNotes["collect.auth_rights"] != nil else { return [] }
            return [Self.findingFromNotes(state: state)]
        }
        ...
        guard surfacePresent else { return [] }

        var evidence: [Evidence] = [
            Evidence(
                type: "auth_summary",
                detail:
                    "authDbPresent=\(auth.authDbPresent.rootstockDescribe) "
                    + "authDbPath=\(auth.authDbPath ?? "nil") "
                    + "authorizationPlists=\(auth.authorizationPlistPaths.count) "
                    + "packageKit=\(auth.packageKitPaths.count) "
                    + "rightsHints=\(auth.rightsHintCount)"
            ),
        ]
        ...
        if sipOff {
            evidence.append(Evidence(type: "compound_sip", detail: "sipEnabled=false"))
        }
        if nonRoot {
            evidence.append(Evidence(type: "compound_priv", detail: "assess running as non-root"))
        }
        evidence.append(
            Evidence(
                type: "honesty",
                detail:
                    "Presence of auth.db / PackageKit is expected on stock macOS. "
                    + "Finding flags the privilege surface for authorized review - not a confirmed misconfiguration."
            )
        )

        let severity: Severity = (sipOff && (auth.authDbPresent == true)) ? .medium : .low
        ...
        return [
            Finding(
                ...
                title: sipOff
                    ...
                    : "Authorization rights / PackageKit privilege surface present",
                severity: severity,
                confidence: .low,
                category: .auth,
                evidence: evidence,
                attackTechniques: ["T1548", "T1548.004", "T1068"],
                remediation: [
                ...
                falsePositiveNotes:
                    "auth.db and PackageKit exist on nearly all macOS installs; severity rises only with compounds (SIP off, custom rights hints).",
                dryRunSafe: true,
                opsecScore: 18,
    ...
    private static func findingFromNotes(state: CollectedState) -> Finding {
        Finding(
            id: id,
            title: "Authorization rights surface (collector note only)",
            severity: .info,
            confidence: .low,
            category: .auth,
            evidence: [
            ...
            attackTechniques: ["T1548"],
            remediation: ["Re-run assess with auth rights collector for typed state"],
            dryRunSafe: true,
            opsecScore: 10,
            esfExpected: ["OPEN"]
        )
