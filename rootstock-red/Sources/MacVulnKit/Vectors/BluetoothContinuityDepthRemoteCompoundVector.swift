...
public struct BluetoothContinuityDepthRemoteCompoundVector: Check {
    ...
    public func evaluate(state: CollectedState, context: EvaluationContext) async throws -> [Finding] {
        ...
        var evidence: [Evidence] = [
            Evidence(type: "bluetooth_continuity_depth_compound", detail: "a=\(a) b=\(b) remote=\(compound.remote) fda=\(compound.fullDiskAccess) sensorThin=\(compound.sensorThin)"),
        ]
        if let s {
        ...
        return [Finding(id: Self.id, title: compound.remote ? "Bluetooth Continuity depth × remote compound" : "Bluetooth Continuity depth × impact compound", severity: severity, category: .misconfig, resolution: .init(evidence: evidence, attackTechniques: ["T1011", "T1200", "T1040"], remediation: [
