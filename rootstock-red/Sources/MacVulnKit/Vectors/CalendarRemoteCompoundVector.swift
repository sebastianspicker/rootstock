...
public struct CalendarRemoteCompoundVector: Check {
    ...
    public func evaluate(state: CollectedState, context: EvaluationContext) async throws -> [Finding] {
        ...
        var evidence: [Evidence] = [
            Evidence(type: "calendar_reminders_compound", detail: "a=\(a) b=\(b) remote=\(remote) fda=\(fda) sensorThin=\(sensorThin)"),
        ]
        if let s {
        ...
        return [Finding(
            id: Self.id,
            title: remote ? "Calendar/Reminders automation × remote compound" : "Calendar/Reminders automation × impact compound",
