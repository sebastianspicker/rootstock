...
public struct FindmyPathPlaneRemoteCompoundVector: Check {
    ...
    public func evaluate(state: CollectedState, context: EvaluationContext) async throws -> [Finding] {
        ...
        var evidence: [Evidence] = [
            Evidence(type: "findmy_path_plane_compound", detail: "a=\(a) b=\(b) remote=\(remote) fda=\(fda) sensorThin=\(sensorThin)"),
        ]
        if let s {
        ...
        return [Finding(
            id: Self.id,
            title: remote ? "Find My path plane × remote compound" : "Find My path plane × impact compound",
