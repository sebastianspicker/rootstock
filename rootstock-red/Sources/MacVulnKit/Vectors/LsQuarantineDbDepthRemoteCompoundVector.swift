...
public struct LsQuarantineDbDepthRemoteCompoundVector: Check {
    ...
    public func evaluate(state: CollectedState, context: EvaluationContext) async throws -> [Finding] {
        ...
        var evidence: [Evidence] = [
            Evidence(type: "ls_quarantine_db_depth_compound", detail: "a=\(a) b=\(b) remote=\(remote) fda=\(fda) sensorThin=\(sensorThin)"),
        ]
        if let s {
        ...
        return [Finding(
            id: Self.id,
            title: remote ? "LS QuarantineEvents depth × remote compound" : "LS QuarantineEvents depth × impact compound",
