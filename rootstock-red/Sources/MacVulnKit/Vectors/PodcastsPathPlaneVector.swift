import Foundation
import RootstockCore

/// Path-to-impact: Podcasts library path residual.
public struct PodcastsPathPlaneVector: Check {
    public static let id = "rootstock.vector.data.podcasts_path_plane"
    public static let cost: CollectorCost = .low
    public init() {}
    public func evaluate(state: CollectedState, context: EvaluationContext) async throws -> [Finding] {
        let s = state.podcastsPathPlane
        let a = s?.podcastsAppPaths.count ?? 0
        let b = s?.podcastsStorePaths.count ?? 0
        let c = s?.podcastsPrefPaths.count ?? 0
        let surface = s?.podcastsSurfacePresent == true || a + b + c >= 2
        let note = state.collectorNotes["collect.podcasts_path_plane"] != nil
        guard surface || note else { return [] }
        guard a >= 1 || b >= 1 else { return [] }
        let remote = state.network?.remoteLoginSSH == true || state.network?.screenSharingARD == true
        let fda = state.tcc?.fullDiskAccessLikely == true
        var evidence: [Evidence] = [
            Evidence(type: "podcasts_path_plane_summary", detail: "a=\(a) b=\(b) c=\(c) remote=\(remote) fda=\(fda)"),
        ]
        if let s {
            for path in (s.podcastsAppPaths + s.podcastsStorePaths + s.podcastsPrefPaths).prefix(10) {
                evidence.append(Evidence(type: "podcasts_path_plane_path", path: path, detail: "Podcasts path plane path"))
            }
            for n in s.notes.prefix(4) { evidence.append(Evidence(type: "podcasts_path_plane_note", detail: n)) }
        }
        evidence.append(Evidence(type: "honesty", detail: "Assess never dumps podcast episode files or account tokens."))
        let severity: Severity = (remote && fda && a + b >= 3) ? .high : ((remote || fda || a + b >= 2) ? .medium : .low)
        return [Finding(
            id: Self.id,
            title: remote ? "Podcasts path plane with remote amplifier" : "Podcasts library path residual",
            severity: severity, confidence: .medium, category: .misconfig, evidence: evidence,
            attackTechniques: ["T1005", "T1083", "T1119"],
            remediation: [
                "Inventory and baseline Podcasts path plane paths via MDM/EDR",
                "Correlate unexpected co-presence with delivery timelines",
                "Prioritize hosts with remote/FDA amplifiers",
                "OPSEC: Rootstock Red never dumps podcast episode files or account tokens",
            ],
            falsePositiveNotes: "Stock paths often exist. Elevate multi-path co-presence with remote/FDA.",
            dryRunSafe: true, opsecScore: 25, esfExpected: ["OPEN", "READ", "EXEC"]
        )]
    }
}
