...
public struct ShellPluginManagerRemoteCompoundVector: Check {
    ...
    public func evaluate(state: CollectedState, context: EvaluationContext) async throws -> [Finding] {
        ...
        var evidence: [Evidence] = [
            Evidence(type: "shell_plugin_manager_compound", detail: "a=\(a) b=\(b) remote=\(remote) fda=\(fda) sensorThin=\(sensorThin)"),
        ]
        if let s {
        ...
        return [Finding(
            id: Self.id,
            title: remote ? "Shell plugin manager dual-use × remote compound" : "Shell plugin manager dual-use × impact compound",
