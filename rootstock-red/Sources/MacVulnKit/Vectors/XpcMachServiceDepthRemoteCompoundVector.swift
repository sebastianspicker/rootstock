...
public struct XpcMachServiceDepthRemoteCompoundVector: Check {
    ...
    public func evaluate(state: CollectedState, context: EvaluationContext) async throws -> [Finding] {
        ...
        var evidence: [Evidence] = [
            Evidence(type: "xpc_mach_service_depth_compound", detail: "a=\(a) b=\(b) remote=\(compound.remote) fda=\(compound.fullDiskAccess) sensorThin=\(compound.sensorThin)"),
        ]
        if let s {
        ...
        return [Finding(id: Self.id, title: compound.remote ? "XPC Mach service depth × remote compound" : "XPC Mach service depth × impact compound", severity: severity, category: .misconfig, resolution: .init(evidence: evidence, attackTechniques: ["T1559", "T1543", "T1055"], remediation: [
