...
Usage:
    python3 scripts/check-neo4j-connection.py [--uri bolt://localhost:7687] [--user neo4j]

Set the password with the NEO4J_PASSWORD environment variable.
...
try:
    ...
    print("ERROR: neo4j driver not installed. Run: uv sync --project graph --extra dev", file=sys.stderr)
    sys.exit(1)
...
def main() -> int:
    ...
    if not password:
        print("ERROR: Neo4j password required via NEO4J_PASSWORD",
              file=sys.stderr)
        return 1
